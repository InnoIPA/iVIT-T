    ...
    # def ss(self):
        try:
            ...
            for order_id,file_val in self.deal_file.items():
                ...
                check_sum = hashlib.md5(open(os.path.join(file_val["source_folder"],file_val["filename"]),'rb').read()).hexdigest()
                with open(check_sum_txt_path) as f:
                    ...
                    if str(check_sum)!=str(data):
                        ...
                        shutil.rmtree(file_val["destination_folder"])
                ...
                with zipfile.ZipFile(os.path.join(file_val["source_folder"],file_val["filename"]), mode='r') as zf:
                    zf.extractall(os.path.join(file_val["source_folder"]))
                # print(file_val["source_folder"])
                #get project info from json .
                json_filename=os.path.splitext(file_val["filename"])[0]+".json"
                ...
                if file_val["ori_project_name"] !="":
                    _t=file_val["ori_project_name"]
                else:
                    _t=file_val["filename_without_Extension"]
                ...
                status = insert_data_from_json("project",project,_temp_iter_map,file_val["rename"],file_val["filename_without_Extension"],new_uuid,file_val["source_folder"])
                ...
                status = insert_data_from_json("workspace",workspace,_temp_iter_map,file_val["rename"],file_val["filename_without_Extension"],new_uuid,file_val["source_folder"])
                ...
                status = insert_data_from_json("color_id",color_id,_temp_iter_map,file_val["rename"],file_val["filename_without_Extension"],new_uuid,file_val["source_folder"])
                if status:
                    os.remove(os.path.join(file_val["source_folder"],file_val["filename"]))
                    shutil.rmtree(file_val["temp_dir"]) 
                    shutil.rmtree(file_val["destination_folder"])   
                ...
                model=db_info['model']
                status = insert_data_from_json("model",model,_temp_iter_map,file_val["rename"],file_val["filename_without_Extension"],new_uuid,file_val["source_folder"])
                if status:
                    os.remove(os.path.join(file_val["source_folder"],file_val["filename"]))
                    shutil.rmtree(file_val["temp_dir"]) 
                    shutil.rmtree(file_val["destination_folder"])
                ...
                for id , val in iteration.items():
                    ...
                    status = insert_data_from_json("iteration"+str(_temp_iter_map[id]),val,_temp_iter_map,file_val["rename"],file_val["filename_without_Extension"],new_uuid,file_val["source_folder"])
                    if status:
                        os.remove(os.path.join(file_val["source_folder"],file_val["filename"]))
                        shutil.rmtree(file_val["temp_dir"]) 
                        shutil.rmtree(file_val["destination_folder"])
                ...
                remove_file(root_path,file_val["destination_folder"])
            ...
            return_info = {
                ...
                "project_uuid": uuid,
                "project_name": file_val["filename"]
            }
