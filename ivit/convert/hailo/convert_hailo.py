...
def main(args):
	...
	input_model = os.path.join( model_dict["train_config"]["save_model_path"], model_name)
	output_dir = os.path.join( model_dict["train_config"]["save_model_path"], 'hailo_model')
	...
	if "classification" in args.config:
		...
		from convert_unet import convert_unet
		convert_unet(input_model, output_dir, export_dir, project_name, model_dict, args.config, shape)
